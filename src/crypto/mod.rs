...
impl std::cmp::PartialEq for JwsSignerEnum {
    fn eq(&self, other: &Self) -> bool {
        match (self, other) {
            (JwsSignerEnum::ES256 { kid: kid_a, .. }, JwsSignerEnum::ES256 { kid: kid_b, .. }) => {
                kid_a.eq(kid_b)
            }
            (JwsSignerEnum::RS256 { kid: kid_a, .. }, JwsSignerEnum::RS256 { kid: kid_b, .. }) => {
                kid_a.eq(kid_b)
            }
            (JwsSignerEnum::HS256 { kid: kid_a, .. }, JwsSignerEnum::HS256 { kid: kid_b, .. }) => {
                kid_a.eq(kid_b)
            }
            _ => false,
        }
...
impl std::cmp::Eq for JwsSignerEnum {}

impl std::hash::Hash for JwsSignerEnum {
    fn hash<H: std::hash::Hasher>(&self, state: &mut H) {
        match self {
            JwsSignerEnum::ES256 { kid, .. }
            | JwsSignerEnum::RS256 { kid, .. }
            | JwsSignerEnum::HS256 { kid, .. } => {
                kid.hash(state);
            }
        }
    }
...
impl fmt::Debug for JwsSignerEnum {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            JwsSignerEnum::ES256 { kid, .. } => f
                .debug_struct("JwsSignerEnum::ES256")
                .field("kid", kid)
                .finish(),
            JwsSignerEnum::RS256 { kid, .. } => f
                .debug_struct("JwsSignerEnum::RS256")
                .field("kid", kid)
                .finish(),
            JwsSignerEnum::HS256 { kid, .. } => f
                .debug_struct("JwsSignerEnum::HS256")
                .field("kid", kid)
                .finish(),
        }
    }
...
impl fmt::Debug for JwsValidatorEnum {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_struct("JwsValidatorEnum").finish()
    }
}

impl JwsValidatorEnum {
    /// Get the KID of this validator if present
    pub fn get_jwk_kid(&self) -> Option<&str> {
        match self {
            JwsValidatorEnum::ES256 { kid, .. } => kid.as_deref(),
            JwsValidatorEnum::RS256 { kid, .. } => kid.as_deref(),
            JwsValidatorEnum::HS256 { kid, .. } => kid.as_deref(),
        }
    }
}
...
impl JwsCompact {
    ...
    pub(crate) fn validate(&self, validator: &JwsValidatorEnum) -> Result<JwsInner, JwtError> {
        match (validator, &self.header.alg) {
            (
                JwsValidatorEnum::ES256 {
                    kid: _,
                    pkey,
                    digest,
                },
...
impl JwsCompact {
    pub fn verify<K: JwsVerifier>(&self, verifier: &mut K) -> Result<JwsInner, JwtError> {
...
impl JwsInner {
    #[cfg(test)]
    pub fn sign_embed_public_jwk(&self, signer: &JwsSignerEnum) -> Result<JwsCompact, JwtError> {
        let jwk = signer.public_key_as_jwk()?;
        self.sign_inner(signer, None, Some(jwk))
    }

    #[cfg(test)]
    pub fn sign(&self, signer: &JwsSignerEnum) -> Result<JwsCompact, JwtError> {
    ...
    pub(crate) fn sign_inner(
        ...
        let alg = match signer {
            JwsSignerEnum::ES256 { .. } => JwaAlg::ES256,
            JwsSignerEnum::RS256 { .. } => JwaAlg::RS256,
            JwsSignerEnum::HS256 { .. } => JwaAlg::HS256,
        };
...
impl TryFrom<&Jwk> for JwsValidatorEnum {
    type Error = JwtError;
    ...
    fn try_from(value: &Jwk) -> Result<Self, Self::Error> {
        match value {
            Jwk::EC {
                ...
                Ok(match crv {
                    EcCurve::P256 => JwsValidatorEnum::ES256 { kid, pkey, digest },
                })
            }
            Jwk::RSA {
                n,
                ...
                Ok(JwsValidatorEnum::RS256 { kid, pkey, digest })
            }
        }
    }
}
...
#[cfg(feature = "openssl")]
impl TryFrom<x509::X509> for JwsValidatorEnum {
    type Error = JwtError;
    ...
    fn try_from(value: x509::X509) -> Result<Self, Self::Error> {
        ...
        let digest = hash::MessageDigest::sha256();
        pkey.ec_key()
            .map(|pkey| JwsValidatorEnum::ES256 {
                kid: None,
                pkey,
                digest,
            })
            .or_else(|_| {
                pkey.rsa().map(|pkey| JwsValidatorEnum::RS256 {
                    kid: None,
                    pkey,
                    digest,
                })
...
impl JwsSignerEnum {
    ...
    pub fn from_es256_jwk_components(x: &str, y: &str, d: &str) -> Result<Self, JwtError> {
        ...
        Ok(JwsSignerEnum::ES256 { kid, skey, digest })
    }
    ...
    pub fn from_hs256_raw(buf: &[u8]) -> Result<Self, JwtError> {
        ...
        Ok(JwsSignerEnum::HS256 { kid, skey, digest })
    }
    ...
    pub fn get_validator(&self) -> Result<JwsValidatorEnum, JwtError> {
        match self {
            JwsSignerEnum::ES256 { kid, skey, digest } => {
            ...
            JwsSignerEnum::RS256 { kid, skey, digest } => {
                ...
                rsa::Rsa::from_public_components(n, e)
                    ...
                    .map(|pkey| JwsValidatorEnum::RS256 {
                        kid: Some(kid.clone()),
                        pkey,
                        digest: *digest,
                    })
            }
            JwsSignerEnum::HS256 { kid, skey, digest } => Ok(JwsValidatorEnum::HS256 {
                kid: Some(kid.clone()),
                skey: skey.clone(),
                digest: *digest,
            }),
    ...
    pub fn from_rs256_der(der: &[u8]) -> Result<Self, JwtError> {
        ...
        Ok(JwsSignerEnum::RS256 { kid, skey, digest })
    }
    ...
    */
    ...
    pub fn get_kid(&self) -> &str {
        match self {
            JwsSignerEnum::ES256 { kid, .. } => &kid,
            JwsSignerEnum::RS256 { kid, .. } => &kid,
            JwsSignerEnum::HS256 { kid, .. } => &kid,
        }
    }

    /// Export this JwsSignerEnum to a DER private key.
...
mod tests {
    use super::{JwsEs256Verifier, JwsRs256Verifier};
    use super::{JwsSignerEnum, JwsValidatorEnum};
    ...
    fn rfc7515_es256_signature_example() {
        ...
        let jwss = JwsSignerEnum::from_es256_jwk_components(
        ...
        let jws_validator = JwsValidatorEnum::try_from(&pkey).expect("Unable to create validator");
    ...
    fn es256_key_generate_cycle() {
        ...
        let jwss = JwsSignerEnum::from_es256_der(&der).expect("Failed to restore signer");
    ...
    fn rs256_key_generate_cycle() {
        let _ = tracing_subscriber::fmt::try_init();
        let jwss = JwsSignerEnum::generate_legacy_rs256().expect("failed to construct signer.");

        let der = jwss.private_key_to_der().expect("Failed to extract DER");

        let jwss = JwsSignerEnum::from_rs256_der(&der).expect("Failed to restore signer");
    ...
    fn rfc7519_hs256_validation_example() {
        ...
        let jws_signer = JwsSignerEnum::from_hs256_raw(&skey).expect("Unable to create validator");
